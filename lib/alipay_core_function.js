...
const detectConfig = (obj) => {
	...
	let publicParam = (env === 'international') ? api_list.international_PublicParam : api_list.publicParam; //公共参数
	...
	for (let i = 0; i < api_list[method].length; i++) {
		if (typeof api_list[method][i] === 'object') {
			let paramExist = 0;
			for (let j = 0; j < api_list[method][i].length; j++) {
				if (obj.biz_content[api_list[method][i][j]]) {
					paramExist = 1;
					break;
				}
			}
		...
		if (!obj.biz_content[api_list[method][i]] && typeof api_list[method][i] !== 'object') {
